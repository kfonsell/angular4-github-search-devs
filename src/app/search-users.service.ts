 ...
 export class SearchUsersService {
   ...
   getUsersByPlaceAndLanguage(place: string, language: string) {
     let url;
     if (place && !language) {
       url = `${this.searchUsersEndPoint}location:${place}&per_page=50`;
     } else if (!place && language) {
       url = `${this.searchUsersEndPoint}language:${language}&per_page=50`;
     } else {
       url = `${this.searchUsersEndPoint}location:${place}+language:${language}&per_page=50`;
     }
